...
pub const DIRECTMAP_BASE: u64 = KERNEL_BASE + 0x0000_4000_0000; // phys→virt linear window
pub const DIRECTMAP_SIZE: u64 = 0x0002_0000_0000;               // 128 GiB direct map

pub const KHEAP_BASE:  u64 = KERNEL_BASE + 0x0002_4000_0000; // kernel heap arena (VA only)
pub const KHEAP_SIZE:  u64 = 0x0000_8000_0000;               // 32 GiB

pub const KVM_BASE:    u64 = KERNEL_BASE + 0x0003_0000_0000; // anon VM (kalloc_pages)
pub const KVM_SIZE:    u64 = 0x0001_8000_0000;               // 96 GiB

pub const MMIO_BASE:   u64 = KERNEL_BASE + 0x0004_8000_0000; // device MMIO VA window
pub const MMIO_SIZE:   u64 = 0x0000_8000_0000;               // 32 GiB

pub const VMAP_BASE:   u64 = KERNEL_BASE + 0x0005_0000_0000; // vmap/ioremap overflow
pub const VMAP_SIZE:   u64 = 0x0001_0000_0000;               // 64 GiB

// Fixmap: small, fixed VA slots for early/temporary mappings (kasan, acpi, etc.)
pub const FIXMAP_BASE: u64 = KERNEL_BASE + 0x0006_0000_0000;
pub const FIXMAP_SIZE: u64 = 0x0000_0100_0000;               // 16 MiB, page-granular slots

// Boot identity window (temporary 1:1 map during bring-up)
pub const BOOT_IDMAP_BASE: u64 = KERNEL_BASE + 0x0006_1000_0000;
...
pub const PERCPU_BASE:   u64 = KERNEL_BASE + 0x0007_0000_0000;
...
pub mod pat {
    ...
    pub const WP:       u8 = 5;
    pub const WB:       u8 = 6;
    pub const UC_MINUS: u8 = 7; // UC- device-like
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RegionKind { Usable, Reserved, Acpi, Mmio, Kernel, Boot, Unknown }

#[derive(Clone, Copy, Debug)]
pub struct Region { pub start: u64, pub len: u64, pub kind: RegionKind }
impl Region { pub const fn end(&self) -> u64 { self.start + self.len } pub const fn is_usable(&self) -> bool { matches!(self.kind, RegionKind::Usable) } }
