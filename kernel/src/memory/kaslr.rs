...
//  - Entropy transcript (RDSEED/RDRAND + TSC jitter + CPUID + optional boot salt)
//  - SHA3-based DRBG (counter mode) for deterministic, audit-friendly randomness
...
impl Range { #[inline] pub const fn contains(&self, x: u64) -> bool { x >= self.lo && x < self.hi } }
...
pub unsafe fn init(policy: Policy) -> Kaslr {
    ...
    h.update(&buf[..used]);
    let transcript = h.finalize_bytes();
    ...
    let mut nb = [0u8; 8]; nb.copy_from_slice(&transcript[8..16]);
    ...
    // 7) Seed per-CPU offsets/canaries deterministically (no mapping here; just recordable randomness)
    seed_percpu(&mut drbg, policy);

    // 8) Proof events (public commitments only)
    //    we encode: slide (low 32 bits in flags) + cpu_count in len; paddr field carries the raw slide
    proof::audit_map(L::KERNEL_BASE, slide, policy.cpu_count as u64, (slide & 0xffff_ffff), CapTag::KERNEL);
    //    commit transcript hash (split into two PhysAlloc events to keep the event format stable)
    let mut part0 = [0u8; 8]; part0.copy_from_slice(&transcript[0..8]);
    let mut part1 = [0u8; 8]; part1.copy_from_slice(&transcript[8..16]);
    let mut part2 = [0u8; 8]; part2.copy_from_slice(&transcript[16..24]);
    let mut part3 = [0u8; 8]; part3.copy_from_slice(&transcript[24..32]);
...
#[inline] pub fn boot_nonce() -> u64 { BOOT_NONCE.load(Ordering::Relaxed) }
...
unsafe fn collect_entropy(out: &mut [u8], boot_salt: Option<[u8;32]>) -> usize {
    ...
    if let Some(s) = boot_salt {
        if out.len() - w >= 32 { out[w..w+32].copy_from_slice(&s); w += 32; }
    }

    // C) RDSEED / RDRAND (with repetition test)
    let mut rng_ok = false;
    if has_rdseed() { w += fill_rdseed_checked(&mut out[w..], &mut rng_ok); }
    if !rng_ok && has_rdrand() { w += fill_rdrand_checked(&mut out[w..], &mut rng_ok); }
...
#[inline] fn has_rdrand() -> bool { let (_a,_b,c,_d)=cpuid(1,0); (c & (1<<30))!=0 }
#[inline] fn has_rdseed() -> bool { let (_a,b,_c,_d)=cpuid(7,0); (b & (1<<18))!=0 }

#[inline] fn cpuid(leaf:u32, sub:u32)->(u32,u32,u32,u32){
    let mut a=leaf; let mut b:u32; let mut c=sub; let mut d:u32;
    unsafe { core::arch::asm!("cpuid", inlateout("eax") a, lateout("ebx") b, inlateout("ecx") c, lateout("edx") d, options(nostack, preserves_flags)); }
    (a,b,c,d)
}
...
unsafe fn write_cpuid_salt(buf:&mut [u8])->usize{
    ...
    let mut o=0;
    for v in [a0,b0,c0,d0,a1,b1,c1,d1,a7,b7,c7,d7] { buf[o..o+4].copy_from_slice(&v.to_le_bytes()); o+=4; }
    o
}

unsafe fn rdrand64()->Option<u64>{ let mut v:u64; let mut ok:u8; core::arch::asm!("rdrand rax; setc dl",out("rax")v,out("dl")ok,options(nostack,preserves_flags)); if ok!=0{Some(v)}else{None}}
unsafe fn rdseed64()->Option<u64>{ let mut v:u64; let mut ok:u8; core::arch::asm!("rdseed rax; setc dl",out("rax")v,out("dl")ok,options(nostack,preserves_flags)); if ok!=0{Some(v)}else{None}}

unsafe fn fill_rdrand_checked(buf:&mut [u8], ok:&mut bool)->usize{
    let mut off=0; let mut last=0u64; let mut reps=0u32;
    while buf.len()-off>=8 {
        if let Some(v)=rdrand64(){ if v==last{reps+=1;} else {reps=0; last=v;} buf[off..off+8].copy_from_slice(&v.to_le_bytes()); off+=8; if reps>8 { break; } }
        else { break; }
    }
    *ok = off>=32 && reps<=8; off
}
unsafe fn fill_rdseed_checked(buf:&mut [u8], ok:&mut bool)->usize{
    let mut off=0; let mut last=0u64; let mut reps=0u32;
    while buf.len()-off>=8 {
        if let Some(v)=rdseed64(){ if v==last{reps+=1;} else {reps=0; last=v;} buf[off..off+8].copy_from_slice(&v.to_le_bytes()); off+=8; if reps>8 { break; } }
        else { break; }
    }
    *ok = off>=32 && reps<=8; off
}
...
unsafe fn fill_tsc_jitter(buf:&mut [u8])->usize{
    ...
    for _ in 0..128 {
        ...
        let b=rdtsc(); let d=b.wrapping_sub(a)^x;
        if buf.len()-off<8 {break;}
        buf[off..off+8].copy_from_slice(&d.to_le_bytes()); off+=8;
    }
    off
}
#[inline] fn rdtsc()->u64{ unsafe{ let mut hi:u32; let mut lo:u32; core::arch::asm!("rdtsc",out("edx")hi,out("eax")lo,options(nomem,nostack,preserves_flags)); ((hi as u64)<<32)|(lo as u64) } }

// ───────────────────────────────────────────────────────────────────────────────
// DRBG: SHA3-256(counter || key) stream; deterministic, simple, audit-friendly
// ───────────────────────────────────────────────────────────────────────────────

struct Drbg { key: [u8;32], ctr: u64 }
impl Drbg {
    fn seed(seed:&[u8;32])->Self{
        // key = H("DRBG\0" || seed || "NONOS")
        let mut h=Sha3_256::new(); h.update(b"DRBG\0"); h.update(seed); h.update(b"NONOS");
        Self{ key: h.finalize_bytes(), ctr: 1 }
    }
    ...
    fn u64(&mut self)->u64 { let mut b=[0u8;8]; self.fill(&mut b); u64::from_le_bytes(b) }
    fn choose_u64(&mut self, bound_exclusive: u64)->u64 {
        ...
        loop { let r=self.u64(); if r<t { return r % bound_exclusive; } }
    }
}
