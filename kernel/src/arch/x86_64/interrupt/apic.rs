...
use crate::memory::mmio::CacheAttr;
...
#[inline(always)] fn rdmsr(ix: u32) -> u64 { unsafe { Msr::new(ix).read() } }
#[inline(always)] fn wrmsr(ix: u32, v: u64) { unsafe { Msr::new(ix).write(v) } }
...
fn has_xapic() -> bool { let (_a,_b,_c,d)=cpuid(1,0); (d & (1<<9))!=0 }
fn has_x2apic() -> bool { let (_a,_b,c,_d)=cpuid(1,0); (c & (1<<21))!=0 }
fn has_tsc_deadline() -> bool { let (_a,_b,c,_d)=cpuid(1,0); (c & (1<<24))!=0 }
...
pub unsafe fn init() {
    ...
    if has_x2apic() {
        ...
        proof::audit_map(va.as_u64(), phys, PAGE_SIZE as u64, (VmFlags::RW|VmFlags::NX|VmFlags::GLOBAL|VmFlags::PCD).bits(), CapTag::KERNEL);
    }
    ...
    proof::audit_phys_alloc(0xA11C_AP1C, 0x1N17_u64, CapTag::KERNEL);
}
...
#[inline] pub fn ipi_one(apic_id: u32, vec: u8) {
    if X2APIC.load(Ordering::Relaxed) {
...
#[inline] pub fn ipi_others(vec: u8) {
    if X2APIC.load(Ordering::Relaxed) {
...
pub fn timer_enable(hz: u32, divider: u8, initial_count_hint: u32) -> bool {
    ...
    if deadline {
        // Program LVT Timer to vector with "TSC-Deadline" mode (bit 18 in xAPIC LVT when supported;
        // in x2APIC, using TSC-deadline MSR is sufficient; set LVT vector with periodic bit clear).
        if X2APIC.load(Ordering::Relaxed) {
            wrmsr(IA32_X2APIC_LVT_TIMER, (VEC_TIMER as u64)); // deadline mode implicitly via IA32_TSC_DEADLINE
        } else {
            // Some xAPICs also honor deadline: use vendor check; otherwise fallback to periodic
        ...
        proof::audit_phys_alloc(0xDEAD_1NEu64, 1, CapTag::KERNEL);
        true
    } else {
        // Periodic fallback
        ...
        proof::audit_phys_alloc(0xPER1_OD1Cu64, hz as u64, CapTag::KERNEL);
        false
    }
}

/// Set a TSC deadline (absolute TSC). Only valid if timer_enable returned true.
#[inline] pub fn timer_deadline_tsc(tsc: u64) { wrmsr(IA32_TSC_DEADLINE, tsc); }
...
fn calibrate_lapic_count(hz: u32, _div_code: u32) -> u32 {
    // Expected ticks per period: LAPIC counts at bus/APIC clock; we approximate via TSC scale.
    // For demo: pick a conservative initial load and refine later via HPET/PIT (see hooks below).
    // 1 kHz ~ 1ms period. Start with large value; adjust empirically in QEMU.
...
#[inline(always)] fn mmio_r32(off: u32) -> u32 { unsafe { core::ptr::read_volatile((mmio_base().as_u64()+off as u64) as *const u32) } }
#[inline(always)] fn mmio_w32(off: u32, v: u32) { unsafe { core::ptr::write_volatile((mmio_base().as_u64()+off as u64) as *mut u32, v) } }
