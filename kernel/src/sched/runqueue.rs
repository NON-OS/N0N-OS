...
pub fn enqueue(tid: TaskId, prio: Priority) {
    ...
    rq.mark(lvl);
    proof::audit_phys_alloc(0xRQ_ENQ, ((tid.0 as u64) << 8) | (lvl as u64), CapTag::KERNEL);
}
...
pub fn dequeue(tid: TaskId) {
    ...
    for lvl in 0..LEVELS {
        if let Some(pos) = rq.q[lvl].iter().position(|&x| x.0 == tid.0) {
            ...
            proof::audit_phys_alloc(0xRQ_DEQ, ((tid.0 as u64) << 8) | (lvl as u64), CapTag::KERNEL);
            return;
        }
    }
}
...
pub fn pick_next() -> Option<(TaskId, Priority)> {
    ...
    proof::audit_phys_alloc(0xRQ_PICK, ((tid.0 as u64) << 8) | (lvl as u64), CapTag::KERNEL);
    Some((tid, prio))
}
...
/// Cooperative yield: place current at tail and select another.
pub fn yield_now(tid: TaskId, prio: Priority) {
    rotate_after_run(tid, prio);
}

/// Put a task to sleep: ensure it’s not on any run queue.
pub fn sleep(tid: TaskId) {
    dequeue(tid);
    // state will be set by scheduler/task layer; no proof spam here
}
...
fn aging_step(rq: &mut CpuRq) {
    ...
    for lvl in (1..LEVELS).rev() {
        if let Some(tid) = rq.q[lvl].pop_front() {
            ...
            rq.mark(dest);
            rq.unmark_if_empty(lvl);
            proof::audit_phys_alloc(0xRQ_AGE, ((tid.0 as u64) << 8) | (dest as u64), CapTag::KERNEL);
        }
    }
}
